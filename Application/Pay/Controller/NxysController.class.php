...
class NxysController extends PayController
{
    ...
    public function skf2(){
        $orderId = I('get.id/s');
        $urltemp = U('Nxys/skf',array('id'=> $orderId),true,true);
        $sk = new \authorize();
    ...
    public function getnums(){
        ...
        file_put_contents('./Data/Nxys.txt', "【".date('Y-m-d H:i:s')."】\r\n".json_encode($data)."\r\n\r\n",FILE_APPEND);
    }
    ...
    public function notifyurl()
    {
       // {"money":"0.02","orderNo":"00005215601542452059553485","sign":"c04997fd816659e9a5740c6691228c9e","deviceId":"346"}
        ...
        file_put_contents('./Data/Nxys.txt', "【".date('Y-m-d H:i:s')."】\r\n".json_encode($data)."\r\n\r\n",FILE_APPEND);
        if($data['sign'] != md5($data['orderNo'].$data['money']."123456")){
            exit("oksuccess");
        }
        $paytime=time()-300;
        $orderWhere['pay_amount'] = I('post.money/f');
        $orderWhere['pay_status'] = 0;
        $orderWhere['account_id'] = I('post.deviceId/s');
        $orderWhere['pay_applydate'] = array('gt',$paytime);
        $orderInfo = M('Order')->where($orderWhere)->select();
        $orderCount = count($orderInfo);
        if(empty($orderInfo)){
        ...
        if($orderCount==1){
            ...
            if($isSystemOrder){
                //不是系统订单
                file_put_contents('./Data/Nxys_ZD.txt', "【".date('Y-m-d H:i:s')."】回调结果：\r\n".json_encode($_REQUEST)."\r\n\r\n",FILE_APPEND);
                echo "非系统订单";die;
            }
            $moneyCheck->deletAccountKey($orderData['account_id'],$data['money']);
            $this->EditMoney($orderData['pay_orderid'], 'Nxys', 0);
            //写入订单号
        ...
        if($orderCount>1){
            //匹配到多个订单
            file_put_contents('./Data/Nxys_ZD.txt', "【".date('Y-m-d H:i:s')."】多个订单回调参数：\r\n".json_encode($_REQUEST)."\r\n\r\n",FILE_APPEND);
            file_put_contents('./Data/Nxys_ZD.txt', "【".date('Y-m-d H:i:s')."】多个订单列表：\r\n".json_encode($orderInfo)."\r\n\r\n",FILE_APPEND);
