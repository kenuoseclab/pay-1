...
class WechatscanController extends PayController
{
    ...
    public function Pay($array)
    {
        ...
        $notifyurl = $this->_site . 'Pay_Wechatscan_notifyurl.html'; //异步通知
        $callbackurl = $this->_site . 'Pay_Wechatscan_callbackurl.html'; //返回通知
        $parameter = array(
            'code' => 'Wechatscan', // 通道名称
            'title' => '微信扫码H5',
            'exchange' => 1, // 金额比例
            'gateway' => '',
            'orderid' => '',
    ...
    public function jsapi($return)
    {
        ...
        if ($arr["result_code"] == "SUCCESS") {
            ...
            $delqr = $QR;
            \QRcode::png($arr['code_url'] , $QR, "L", 20);
            $this->_site.$QR;
            $this->assign("imgurl", $this->_site.$QR);
            $this->assign("out_trade_id", $return['orderid']);
            $this->assign("money", $return['amount']);
            $this->assign("title", $return['subject']);
            $this->assign("created_at", date('Y-m-d H:i:s' , strtotime($return['datetime'])));
    ...
    public function callbackurl()
    {
        ...
        if($find_data['pay_status'] <> 0){
            $this->EditMoney($_REQUEST['orderid'], 'Wechatscan', 1);
        ...
        if($return_info){
            ...
            if($return_info['return_code'] == 'SUCCESS' && $return_info['result_code'] == 'SUCCESS'){
                ...
                if ($my_sign == $return_info['sign']) {
                    $this->EditMoney($return_info['out_trade_no'], 'Wechatscan', 0);
                    exit('SUCCESS');
                }else{
                    exit('FAIL');
                }
