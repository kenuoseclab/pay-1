...
class ChannelController extends UserController
{
    ...
    public function channelAccount(){
        // echo "这是一个子账户添加";
        $account_code = I("request.id/s");
        $zfb = M("Channel")->where(['id'=>$account_code])->find();
    ...
    // 保存
    public function saveEditAccount2()
    {
        if (IS_POST) {
            $id                     = I('post.id', 0, 'intval');
            $papiacc                = I('post.pa/a');
