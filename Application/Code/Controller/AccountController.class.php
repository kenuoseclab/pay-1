...
// 上传成功
    ...
    public function channelFinance()
    {
        ...
        $payMemberid = session('code_auth.uid') + 10000;
    ...
    public function google()
    {
        ...
        if (!IS_POST) {
            ...
            if($google_token == '') {
                $secret = session('code_google_secret_key') ? session('user_google_secret_key') : $ga->createSecret();
            ...
            $google_secret_key = session('code_google_secret_key');
            ...
            if(false === $ga->verifyCode($google_secret_key, $googlecode, C('google_discrepancy'))) {
                ...
                if(FALSE !== $re) {
                    session('code_google_auth', $googlecode);
    ...
    public function dochongzhi(){
        if(IS_POST){
            ...
            $pay_notifyurl = $this->_site."Code_Account_cznotify.html";   //服务端返回地址
            $pay_callbackurl = $this->_site."Code_Account_czcallback.html";  //页面跳转返回地址
    ...
    public function cznotify(){
        ...
        if ($sign == $_REQUEST["sign"]) {
            if ($_REQUEST["returncode"] == "00") {               
                ...
                $moneychange_data = [
                        'userid'     => $memberid,
                        'ymoney'     => $memberinfo['balance'], //原金额或原冻结资金
                        'money'      => $ReturnArray["amount"],
                        'gmoney'     => $memberinfo['balance']+$ReturnArray["amount"], //改动后的金额或冻结资金
                        'datetime'   => date('Y-m-d H:i:s'),
                        'tongdao'    => $order_info['pay_bankcode'],
                        'transid'    => $order_info["pay_orderid"],
                        'orderid'    => $order_info['out_trade_id'],
                        'contentstr' => $order_info['out_trade_id'] . '码商充值',
                        'lx'         => 19,
                        't'          => $order_info['t'],
                    ];
                $moneychange_result = $this->MoenyChange($moneychange_data); // 资金变动记录
                if(M('member')->where(['id'=>$memberid])->setInc('balance',$ReturnArray["amount"])){
