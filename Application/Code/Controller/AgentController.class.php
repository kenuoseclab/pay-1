...
class AgentController extends UserController
{
    ...
    public function exportuser()
    {
        ...
        $map['parentid'] = array('eq', session('code_auth.uid'));
        $regdatetime     = urldecode(I("request.regdatetime"));
        if ($regdatetime) {
            list($cstime, $cetime) = explode('|', $regdatetime);
    ...
    public function saveUserRate()
    {
        if (IS_POST) {
            ...
            foreach ($rows as $key => $item) {
                ...
                //验证要设置的是代理还是码商
                $groupid=M("Member")->where(['id'=>$userid])->getField("groupid");
                if($groupid==8){
                    if($item['feilv'] > $agent_rate['feilv']) {
                        $this->ajaxReturn(['status' => 0, 'msg'=> 'T+1费率不能高于码商成本！']);
                    }
                    if($item['t0feilv'] > $agent_rate['t0feilv']) {
                        $this->ajaxReturn(['status' => 0, 'msg'=> 'T+0费率不能高于码商成本！']);
                    }
                }else{
                    if($item['feilv'] < $agent_rate['feilv']) {
    ...
    public function saveUser()
    {
        ...
        $current_user = session('code_auth');
    ...
    private function getDayReconciliation($begin, $end) {
        ...
        if(empty($data) || (!empty($data) && diffBetweenTwoDays(date('Y-m-d'),$date)<=3)) {//3天内账单实时更新数据
            ...
            //成功总额
            $data['order_success_amount'] = M('Order')->where(['owner_id'=>$this->fans['uid'],'pay_successdate' => ['between', [strtotime($begin), strtotime($end)], 'pay_status' => ['in', '1,2']]])->sum('pay_amount');
            if($insertFlag) {
    ...
    public function incrMoney()
    {
        ...
        if (IS_POST) {
            ...
            if ($cztype == 3) {
                ...
                if($this->fans['balance']<$bgmoney){
                    $this->ajaxReturn(['status' => 0, 'msg' => '您账户余额不足']);
                }
                $data2['balance']=array('exp', "balance-" . $bgmoney);
                $gmoney2          = $this->fans['balance'] - $bgmoney;
            } elseif ($cztype == 4) {
                $data["balance"]  = array('exp', "balance-" . $bgmoney);
            ...
            $arrayField2  = array(
                ...
                "orderid"    => "",
                "lx"         => $cztype, // 增减类型
                "contentstr" => $contentstr . '【操作下级余额变动:' . $date . '】',
            );
